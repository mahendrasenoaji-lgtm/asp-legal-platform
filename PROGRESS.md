...
| 3 Frontend | **Done** — real Next.js 14 App Router + TypeScript app in `app/`, 59 static routes, `npm run build` passes. Static HTML prototype in `prototype/` kept only for reference |
| 4 CMS / Backend | **Done (locally)** — `db/schema.sql` migrated + seeded against a local Postgres 16 (`asp_legal_dev`), all 4 integrity guards verified to actually reject bad rows. CMS editor (Payload) still not installed; `lib/data.ts` still reads `data/*.json`, not this database |
...
2. **Wire the database into the app** — `lib/data.ts` currently reads `data/*.json` directly.
   Pointing it at the seeded Postgres schema instead is the real CMS-swap step for Phase 4.
...
- Next.js app: `npm install && npm run dev` (or `npm run build` to check the 59-route
  static generation still passes).
